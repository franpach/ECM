...
TCP_PORT = 5005 # puerto por defecto donde escucha el servidor
BUFFER_SIZE = 1024 # tamanio maximo del buffer
CLIENT_FILE = "clients.json" # Fichero que contiene la informacion de los clientes
MODEL_FILE = "model.h5" # Fichero en el que se copia el modelo enviado por el cliente
...
def check_active(clientid):
        ...
        with open(RESOURCES_FILE, "r") as csv_file:
                reader = csv.reader(csv_file)
                for s in reader:
                        if s[5] == clientid and s[6] == "analisis":
                                name = s[0]
                                active = 1
                                break
        if active == 0:
...
def actualizar_recursos(json_obj, resources, name):
        ...
        if new_min < (cpu_disp + sum_rest):
                if new_max <= cpu_disp:
                        new_service = [name,new_min,new_max,new_max,round_down((new_max-new_min),2),client_id,"analisis"]
                else:
                        # calculamos una media de cuanto podriamos darle (su max - min / 2)
                        new_asig = round_down((new_max + new_min) / 2, 2)
                        if new_asig <= cpu_disp:
                                new_service=[name,new_min,new_max,new_asig,round_down((new_asig-new_min),2),client_id,"analisis"]
                ...
                if json_obj['cpus']['min'] < (resources[0] + resources[2]):
                        ...
                        with open(MODEL_FILE, 'wb') as f:
                                while True:
                                        data = conn.recv(BUFFER_SIZE)
                                        f.write(data)
                                        if len(data) < BUFFER_SIZE: #cuando se recibe un paquete de menor tamanio se sabe que se acaba el fichero
                                                break
                        ...
                        if check_container[0] == 0:
                                ...
                                command = "docker run -d -t -p {} --name {} --cpus={} elswork/tensorflow-diy".format(map, name, new_cpu)
                        else:
                                command = "docker start {}".format(name)
                        print(command)
                        output = subprocess.check_output(command, shell=True)
                        print(output)
                        command = "docker cp {} {}:/".format(MODEL_FILE, name)
                        print(command)
                        output = subprocess.check_output(command, shell=True)
                        print(output)
                        ...
                        if batch == 1:
                                number_values = json_obj["number_values"]
                                command = 'docker exec -d -t {} python3.5 send_inference_batch.py "{}" {} {}'.format(name, order, number_values, clientid)
                        else:
                                command = 'docker exec -d -t {} python3.5 send_inference.py "{}" {}'.format(name, order, clientid)
                        subprocess.check_output(command, shell=True)
